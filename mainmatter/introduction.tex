...
Current tools such as the DAKOTA system \ctm{[PAUL: you might need to look into citation formats to get in-line citations properly surrounded by parentheses; if in doubt, ask Adarsh :) ]} \ctm{(}\cite{adams2009dakota}\ctm{)} 
\ctm{make advances towards a generalized framework that provides modelers}
% CTM Original: allow modelers access to 
...
scientific models from free text. One such method is presented in the Eidos, INDRA, and Delphi \ctm{PAUL: put in parens:} \cite{EidosIndraDelphi} \ctm{pipeline} % software tool 
