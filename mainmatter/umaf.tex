...
We define a function network to be a graphical model that represents variables as nodes that are then wired together by the functions that determine the state of a variable as a function of other variables. The computations performed in function nodes can be inspected as necessary.
...
Assignment statements represented in an AST have an explicit variable being assigned and a computation used to perform the assignment.
This form easily allows the PA module to extract and separate the computation as a \textit{black-box} function, associated with the variable being assigned, and added to the function collection.
...
In these cases the Equation Reading (ER) module can be used to help determine how to correctly ground variables to those found in the associated publication.
...
The example outputs in this figure were generated by running UMAF over the PETPT evapo-transpiration model mentioned in the introduction.
