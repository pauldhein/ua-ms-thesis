...
\begin{algorithm}
  \caption{Function network wiring algorithm}
  \label{alg:func_net_wiring}
  \begin{algorithmic}[1]
    \Procedure{WireNetwork}{$\mathcal{C},~\mathcal{F}$} \Comment{Creates a function network}
    \State $root \gets \mathcal{C}_{start}$
    \State $N \gets (V,~E,~P)$
    \State \Call{WireContainer}{$root,~N,~\mathcal{C},~\mathcal{F}$}
    \State \Return $N$
    \EndProcedure

    \Procedure{WireContainer}{$C,~N,~\mathcal{C},~\mathcal{F}$}
      \ForAll{$s \in C_{statements}$}
        \If{$type(s) = call$}
          \State $C^{'} \gets \mathcal{C}(s_{name})$
          \State $N^{'} \gets (V^{'},~E^{'},~P^{'})$
          \State $N^{'}_{inputs} \gets \{ N.V(v) \mid v \in s_{inputs} \}$
          \State \Call{WireContainer}{$C^{'},~N^{'},~\mathcal{C},~\mathcal{F}$}
          \State $N.P.append(N^{'})$
          \State $N.V \gets \{ N^{'}.V(v) \mid v \in N^{'}_{outputs} \}$
          \State $N.E \gets \{ (N.V(v),~N^{'}.V(v^{'})) \mid v \in s_{outputs},~v^{'} \in N^{'}_{outputs} \}$
        \Else
          \State $fn \gets \mathcal{F}(s_{name})$
          \State $N.V.append(s_{inputs},~fn,~s_{output})$
          \ForAll{$i \in s_{inputs}$}
            \State $N.E.append((i,~fn))$
          \EndFor
          \State $N.E.append((fn,~s_{output}))$
        \EndIf
      \EndFor
      \State \Return
    \EndProcedure
...
The algorithm to conduct this process is shown in Figure~\ref{alg:mgd_algorithm}.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \Procedure{MGD}{$G$}
    \State $L_{curr} \gets \{n \mid n \in G_{nodes} \land isInputNode(n) \}$
    \State $d \gets 0$
    \While{$L_{curr} \neq \emptyset$}
      \ForAll{$l \in L_{curr}$}
        \State $l_{dist} \gets d$
      \EndFor
    	\State \textit{Mark all members of} $L_{curr}$ \textit{with} $e$
    	\State $L_{curr} \gets \left\{ c \mid c \in \bigcup\limits_{n~\in~L_{curr}}^{} children(n) \right\}$ \Comment{Collect all children of $L_{curr}$}
      \State $e \gets e + 1$
    \EndWhile
    \State $N_L \gets \{(n_{name},~n_{dist}) \mid n \in G_{nodes} \}$
    \Return $N_L$
    \EndProcedure
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
  \caption{GrFN Stack Execution}
  \label{alg:stack_execution}
  \begin{algorithmic}[1]
    \Procedure{Execute}{G, S, I}
      \State $G_{inputs} \gets I$
      \ForAll{$stack\_level \in S$}
        \ForAll{$fn \in stack\_level$}
          \State $n \gets$ \Call{$fn$}{$fn_{inputs}$}
          \State $G.V(fn_{output}) \gets n$
        \EndFor
      \EndFor
      \State \Return $G_{output}$
    \EndProcedure
  \end{algorithmic}
\end{algorithm}
...
In Table~\ref{prog_idioms} a list of imperative programming idioms is shown as well as the current GrFN coverage of the idiom.
...
Table~\ref{pa_eval_table} shown below records the results for the sample study experiment in terms of which Fortran files were able to be parsed and how accurate the GrFN translation was for the Fortran files that were able to be translated to GrFN.
