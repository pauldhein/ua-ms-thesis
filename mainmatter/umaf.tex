...
In order to accomplish this task, UMAF will use the abstract syntax tree (AST) \citep{aho1986dragonBook} of the original source code program as well as variable grounding information from supporting texts in order to build executable models that are linked to real-world variables.
Once the variables in each of the competing models are all linked correctly to the real-world variables they represent the models can be compared to one another upon the inclusion and wiring of the various variables that can be present to attempt to describe the real-world system.
In this chapter I will discuss how the many pieces involved in this framework that accomplish the final goal of creating the executable and comparable models described above.
This chapter will begin with a background section that covers the modules of the AutoMATES framework that provide input to UMAF.
Once the inputs to UMAF have been fully covered I will proceed to discuss how UMAF utilizes these inputs to create executable models, named Grounded Function Networks (GrFN), that are linked to real-world variables.
The main purpose for creating these comparable and executable models is to facilitate model analysis, and many of the model analysis methods we seek to support require probabilistic models that allow for probabilistic inference and sampling techniques to be performed on the model.
...
Finally, the chapter will conclude with an evaluation study that discusses the current program idiom coverage of the UMAF framework as a means to evaluate what types of scientific models that are currently found in source code can be handled by UMAF.
...
\section{An Overview of the Program Analysis Pipeline\label{sec:pa_overview}}
One of the crucial inputs to UMAF is the extracted AST representation of a source code program provided by the Program Analysis Pipeline (PAP). The pipeline extracts the AST associated with a scientific model using the following process:
\begin{enumerate}
  \item Parse the original Fortran parser into a simple AST using the Open Fortran Parser (OFP) % TODO: ADD CITATION
  \item Translate the AST into a Python source code program
  \item Separate out each individual value computation into an unnamed lambda function containing the computation.
  \item Associate each lambda function with a location in a straight-line-code representation that denotes the location of the computation in the original program.
  \item Deduce update indices for the original program variables so that variable updates in the model can be tracked
  \item Output all of the separated lambda computations as well as the straight-line-code representation of the original program.
\end{enumerate}
