...
While large variations in syntax exist among the various programming languages used to encode scientific models, they can all be abstracted to a unifying abstract syntax tree (AST) representation. \cite{aho1986dragonBook}
...
A network is a directed acyclic graph that contains a set of source nodes and a set of sink nodes. \cite{bondy1976graph}
...
A well defined methodology for performing inference over DAGs is to establish the DAG as a bayesian network \cite{bishop2006pattern}, a type of probabilistic graphical model, and then to use the robust set of methods associated with bayes nets to solve inference problems.
...
However, we can view a GrFN CG as a Causal Analysis Graph (CAG) that allows us to visualize only the variable relationships present in the GrFN.
...
\begin{figure}[!htbp]
    \label{petpt_cag}
    \centering
    \includegraphics[clip,width=.8\columnwidth]{PETPT_GrFN_CAG}%
    \caption[PETPT GrFN CAG]{Causal analysis graph view of the PETPT evapo-transpiration model.}
\end{figure}

\begin{figure}[!htbp]
    \label{petasce_cag}
    \centering
    \includegraphics[clip,width=0.8\columnwidth]{PETASCE_GrFN_CAG_smaller}%
    \caption[PETASCE GrFN CAG]{Causal analysis graph view of the PETASCE evapo-transpiration model.}
\end{figure}
\FloatBarrier
...
For the GrFN CG to be a Dynamic Bayes Net (DBN) we need the probability distribution of the output variable to be a product of all of the input probability distributions.\cite{pearl2009causality}
...
The order of computation between function nodes implies that the call stack is equivalent to a partially ordered set (poset) \cite{simovici2008miningTools} upon the function nodes.
...
First a control flow graph (CFG) \cite{allen1970CFG} must be extracted from the GrFN CG.
As defined above, the GrFN CG is bipartite with respect to the variable and function nodes, such that no variable node is adjacent to another variable node and vice-versa for a function node. \cite{bondy1976graph}
...
In order to accomplish the tasks outlined above, I will introduce a new construct, known as a Forward Influence Blanket (FIB). A FIB is a specific instance of a Markov blanket \cite{pearl2009causality}, derived from a GrFN computation graph, that can be used for forward analysis.  % TODO: possibly define forward analysis and markov blanket
...
Using the set of shared nodes and the output node we can discover all computation paths that lead to the output node by doing an exhaustive path search from each shared node to the output node \cite{sedgewick2002algorithmsInC}.
