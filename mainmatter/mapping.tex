...
\ctm{PAUL: The following description gets a bit awkward: you're introducing a lot of technical jargon along with implementation detilas (JSON files). First help your reader to understand what GrFN is attempting to capture: what the variables are and how their states are determined as a function of other variables (i.e., functions); lambda functions are the representations of these functions that assign variable states as a function of other variables...}
...
\begin{figure}[!tbp]
  ...
  \caption[GrFN Computation Graph Examples]{Two examples of GrFN computation graphs computation graphs (CGs). These %The 
CG representations include the variable nodes, represented as circles, named via the names extracted in the source code and including an index representing the update level of the variable found in the source code. Function nodes are shown as squares with a single letter representing the type of function contained at the node. Possible choices are assignment (A), conditional (C), decision (D), or literal (L). Notice that in both CGs the output nodes are variable nodes the inputs of the network can either be variable nodes or literal assignment nodes.}
\end{figure}
\FloatBarrier
